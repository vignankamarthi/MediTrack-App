...
# Page configuration
st.set_page_config(
    page_title="Medication Administration | MediTrack", 
    page_icon="🏥", 
    layout="wide"
)
...
# Page title
st.title("Medication Administration")
st.subheader("Record and track medication administration")
...
tab1, tab2, tab3 = st.tabs(["Administration Records", "Record New Administration", "Medication Schedule"])

# Tab 1: Administration Records
with tab1:
    # Filter section
    ...
    with col1:
        filter_patient = st.selectbox("Filter by Patient", patient_options, key="filter_patient_1")
    
    with col2:
        filter_date = st.date_input("Filter by Date", datetime.now(), key="filter_date_1")
    
    with col3:
        search_term = st.text_input("Search Medications", placeholder="Enter medication name...", key="search_med_1")
...
# Tab 2: Record New Administration
with tab2:
    ...
    with st.form("med_admin_form"):
        ...
        selected_patient = st.selectbox("Select Patient", patient_options, key="patient_select_2")
        ...
        selected_medication = st.selectbox("Select Medication", medication_options, key="med_select_2")
        ...
        with col1:
            admin_date = st.date_input("Administration Date", datetime.now())
            admin_time = st.time_input("Administration Time", datetime.now().time())
        
        with col2:
            route = st.selectbox("Administration Route", ["Oral", "Intravenous", "Intramuscular", "Subcutaneous", "Inhalation"])
            dosage = st.text_input("Dosage", placeholder="Enter dosage amount")
        ...
        if not patient_results:
            ...
            selected_result = st.selectbox("Related Lab Result", lab_result_options, disabled=True)
            result_id = None
        else:
            lab_result_options = [f"{r['test_name']} - {r['result_value']}" for r in patient_results]
            selected_result = st.selectbox("Related Lab Result", lab_result_options)
        ...
        # Notes
        notes = st.text_area("Administration Notes", placeholder="Enter any notes about this administration")
...
with tab3:
    ...
    patients = get_patient_list()
    patient_options = [f"{p['first_name']} {p['last_name']} (#{p['patient_id']})" for p in patients]
    selected_patient = st.selectbox("Select Patient", patient_options, key="patient_select_3")
    ...
    for i, time_slot in enumerate(schedule_times):
        ...
        # Noon medications
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.button("Mark as Administered", use_container_width=True)
            with col2:
                st.button("Delay Administration", use_container_width=True)
...
col1, col2 = st.columns(2)
with col1:
    st.button("Administer PRN Medication", use_container_width=True)
with col2:
    st.button("View Administration History", use_container_width=True)
...
# Footer
st.caption("© 2025 MediTrack - Medication Administration")

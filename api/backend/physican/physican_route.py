########################################################
# Routing Connection Matrices for USER PERSONA: PHYSICAN
# 
...
# Create a new Blueprint object for the Physican User Persona
# Allows for a variable to store a collection of routes.
physician = Blueprint('physican', __name__)

#--------------------[Physican User Persona ~ James #1 ]----------------------


# Produce all treament outcomes with every known effectiveness rating from the system 
@physician.route('/treatment-outcomes', methods=['GET'])
def get_treatment_outcomes():
    cursor = db.get_db().cursor()
    cursor.execute(''' SELECT treatOutcome.outcome_id, treat.treatment_name, treatOutcome.outcome_name, 
                   treatOutcome.outcome_name, treatOutcome.description, treatOutcome.is_postive, condTreatProto.effectiveness_rating
                   FROM TREATMENT_OUTCOMES treatOutcome
                   JOIN TREATMENT treat ON = treat.treatment_id = treatOutcome.treatment_id
                   LEFT JOIN condition_treatment_protocol condTreatProto ON condTreatProto.treatment_id = treatOutcome.treatment_id 
                   ''')
...
@physician.route('/medical-conditions', methods=['GET'])
...
@physician.route('/condition-treatment-protocol/<condition_id>', methods=['GET'])
...
@physician.route('/medication-records/<patient_id>', methods=['GET'])
...
@physician.route('/healthcare-providers', methods=['GET'])
...
@physician.route('/clinical-protocols', methods=['GET'])
...
@physician.route('/clinical-protocols', methods=['POST'])
...
@physician.route('/clinical-protocols/<protocol_id>', methods=['GET'])
...
@physician.route('/clinical-protocols/<protocol_id>', methods=['UPDATE'])
...
@physician.route('/patient-treatment-records/', methods=['GET'])
